...
# Set an arg for the application we want to build (can be passed in / overriden at build time)
# See: https://docs.docker.com/engine/reference/builder/#arg
ARG my_app=aspnet5api
ARG my_test=${my_app}Tests

# Proxy ENV should be set in an env-file at runtime and passed in as build-args at build-time
# See: https://docs.docker.com/engine/reference/commandline/run/#set-environment-variables-e-env-env-file
#      https://docs.docker.com/compose/env-file/

# Create an app user to run containers as (running as root / uid is not a good idea)
# This may require a user of the same uid / gid if bind mounting to the host filesystem
ARG user=app
ARG group=app
ARG uid=2101
ARG gid=2101
...
COPY ./project.json /opt/app/src/${my_app}/
...
# Running dnu restore in the test project pulls in all nuget deps picked up from
...
COPY . /opt/app/src/${my_app}
# COPY test/${my_test} /opt/app/test/${my_test}

# Adding volumes if needed
VOLUME src/${my_app}/wwwroot/Uploads /opt/app/src/${my_app}/wwwroot/Uploads
